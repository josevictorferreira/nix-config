  ...
  hermesAgentModule =
    ...
    in
    {
      ...
      config = {
        ...
        jvf = {
          programs.hermes-agent.username = lib.mkDefault config.jvf.core.username;

          wrappers.users.${username}.programs.hermes.packages = [
          ...
          home.users.${username}.items.".hermes" = {
            kind = "dir";
            mode = "seed";
...
in
{
  flake.modules.nixos.programs-hermes-agent = hermesAgentModule;
  flake.modules.darwin.programs-hermes-agent = hermesAgentModule;
}
