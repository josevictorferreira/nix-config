...
let
  ...
  qt5ctConf = ''
    [Appearance]
    color_scheme_path=/home/josevictor/.config/qt5ct/colors/Catppuccin-Mocha.conf
    custom_palette=true
  ...
  qt5ctConfigDir = pkgs.stdenv.mkDerivation {
    pname = "qt5ct-config";
    version = "1.0.0";

    installPhase = ''
      mkdir -p $out/qt5ct/colors/
      echo "${qt5ctConf}" > $out/qt5ct/qt5ct.conf
      echo "${catpuccinMocha}" > $out/qt5ct/colors/Catppuccin-Mocha.conf
      echo "${catpuccinLatte}" > $out/qt5ct/colors/Catppuccin-Latte.conf
      cp -r ./* $out/
    '';
  };
