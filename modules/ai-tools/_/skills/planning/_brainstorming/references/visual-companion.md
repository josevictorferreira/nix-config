...
**Paths in this guide are relative to this skill's own directory** (e.g.
`~/.claude/skills/brainstorming/`, `~/.config/opencode/skills/brainstorming/`,
`~/.pi/agent/skills/brainstorming/`) — not to the project you are working in.
Resolve them against the skill directory, or `cd` there first. The scripts are
installed read-only without the executable bit, so always invoke them through
`bash` as shown below. `node` must be on `PATH`.
