...
{
  name = "self-learning";
  description = "End-of-session learning loop: distill session friction into AGENTS.md/CLAUDE.md lessons and improve the skill files that were exercised. Use when wrapping up a session that involved real friction (long debugging, repeated wrong approaches, surprising behavior), when the user says \"self-learn\", \"what did you learn\", \"record lessons\", or \"improve the skills you used\", or before finishing substantial work whose hard-won knowledge would otherwise be lost. Do NOT use for trivial or purely conversational sessions.";
  prompt = ''
    # Self-Learn
    ...
    This skill is self-invokable: run it on your own initiative at the end of a session with real friction, without waiting for the user to ask. If it turns out nothing clears the quality bar, say so briefly and stop — that is the expected outcome for most sessions.
