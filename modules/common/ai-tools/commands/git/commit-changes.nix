{ config, lib, ... }:
let
  cfg = config.jvf.aiTools.commands."commit-changes";
  commandOptions = {
    name = "Commit Changes";
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    jvf.programs.opencode.commands."commit-changes" = commandOptions;
    jvf.programs.claudecode.commands."commit-changes" = commandOptions;
  };
}
